...
## Targets other than PostgreSQL (Amazon Redshift, Iceberg on S3)

| ID | Situation | What goes wrong naively | Rule | Test |
|---|---|---|---|---|
| IC-45 | The converted SQL fragments target Amazon Redshift or Iceberg (Spark / Athena) instead of PostgreSQL | PostgreSQL-only rewrites (`ISNULL` → `COALESCE` is fine, but `GETDATE()` → `CURRENT_TIMESTAMP` is unnecessary on Redshift; `DATEADD` stays on Redshift and must change on Spark) and PostgreSQL-only types slip into a warehouse or lake load | `infa_sql_tool.py check --target redshift\|iceberg` keeps the constructs the target supports and runs the target skill's residual-SQL linter (`redshift_tool` RS rules, `iceberg_tool` IB rules) on every fragment; the steering of that target applies to the conversion | auto |
| IC-46 | Connection, owner and datatype mapping for a non-PostgreSQL target; writing Iceberg tables from PowerCenter | the injected XML still says `Microsoft SQL Server`, `dbo` and native SQL Server types; a session tries to write Iceberg directly | `inject --map` with the target map (`params/redshift_map.json`, `params/iceberg_map.json`): Redshift through PowerExchange for Amazon Redshift or ODBC, schema instead of `dbo`; Iceberg is not written by PowerCenter — the mapping lands files on S3 (PowerExchange for Amazon S3) and the Glue MERGE job of `sql-conversion-iceberg` loads the Iceberg table; Athena is read-only for lookups (ODBC) | auto |
