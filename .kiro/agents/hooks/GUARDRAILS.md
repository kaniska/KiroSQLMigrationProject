...
| HOOK-06 | Windows readiness (static, no PowerShell needed): `supporting-files/check_windows_readiness.py` — twins present with BOM/CRLF and `-ExecutionPolicy Bypass` launchers, balanced PowerShell syntax, referenced files resolve, hand-written twins reference the same tests, catalogs and rule prefixes as their `.sh`, generated twins and Windows agents in sync, hooks run `python -X utf8`, `mcp.json` has Windows variants of local Python servers, `.gitattributes` keeps line endings, every test re-executes in UTF-8 mode on Windows | auto |
