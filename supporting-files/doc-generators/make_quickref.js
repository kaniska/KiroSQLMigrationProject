...
const col1 = [
  ...
  grid([0.31, 0.44, 0.25], ['Skill', 'Does', 'Proven by'], [
    ...
    ['`sql-reporting`', 'Report and analytics SQL on PostgreSQL (executed), Redshift, Athena, Spark (dialect-checked, `report_tool.py check --target`)', '15 patterns · 24 RQ · 18 RD'],
    ['`informatica-etl-conversion`', 'PowerCenter XML with SQL Server SQL → PostgreSQL, Redshift (`check --target`, maps) or the Iceberg lake (S3 landing + Glue job)', '5 mappings · 46 IC'],
  ...
  grid([0.28, 0.72], null, [
    ...
    ['`migration.md` · `reporting.md`', 'PostgreSQL rules: type/syntax maps, hard [H], parity [P] · reporting rules across targets [RE] + dialect catalog RD'],
  ...
  h('Agent and hooks'),
  p('`sql-migration-agent` (conversion assistant: groups 1 + 3 — asks consumer, target, contract, metadata, security, test target, then picks the skill) · `sql-reporting-agent` (reporting SQL assistant on PostgreSQL / Redshift / Athena / Spark) · `*-windows` twins. Prompts: `prompts/examples.md`; catalog + tests: `AGENTS.md` (AG-01..08); `bash supporting-files/verify_agents.sh [--smoke]`.'),
  grid([0.28, 0.72], null, [
...
const col2 = [
  ...
  grid([0.36, 0.64], ['Tool', 'Commands'], [
    ...
    ['`infa_sql_tool.py`', '`extract` · `check --target postgres|redshift|iceberg` · `inject --map` · `render`'],
    ['`report_tool.py`', '`check --target postgres|redshift|athena|spark` · `toolbox --need` · `examples` · `targets`'],
  ...
  grid([0.52, 0.48], ['Linux / macOS', 'Windows'], [
    ...
    ['`python3 supporting-files/check_windows_readiness.py`', 'static Windows readiness check (HOOK-06): twins, launchers, syntax, drift, mcp.json'],
    ['`bash supporting-files/verify_agents.sh [--smoke]`', '`supporting-files\\verify_agents.cmd`'],
...
const col3 = [
  ...
  p('The kit never creates AWS resources. Nothing is applied to a live target: DDL, patches and jobs are packaged for the normal release path. MCP: inbound servers (PostgreSQL, SQL Server, AWS docs) configured but disabled; outbound placeholder `sqlmigration-kit` (`supporting-files/mcp/kit_mcp_server.py`) serves assess / check / compare / toolbox / audit read-only (MCP-01..04).', { after: 8 }),
  ...
  h('Tests — 694 checks, all passing'),
  grid([0.66, 0.34], null, [
    ['Project suites (Aurora)', '153'],
    ['sql-conversion (SQL) · migkit + governance (unit)', '257 · 41'],
    ['sql-reporting (SQL + dialects) · Informatica (unit + SQL)', '88 + 5 · 29 + 38'],
    ['assessment · Redshift · Iceberg · schema · change (unit)', '12 · 13 · 10 · 8 · 9'],
    ['Agent hooks (incl. Windows readiness) · agents · kit MCP server', '19 · 8 · 4'],
  ]),
  p('Coverage gate: every H/P/CC, RQ/RP/RD, IC, SEC/LOG/SVC/GOV, MA/RS/IB/SC/CP, GRD/HOOK/AG/MCP rule has a tagged test. Redshift/Athena/Glue paths run against a stub AWS CLI; live when test resources are configured.', { after: 8 }),
  h('Documents (docs/)'),
  ...
  b('**Next:** data validation at a snapshot on all targets · Spectrum / managed Iceberg serving · **MCP packaging (placeholder started)** · **AI-DLC integration (placeholder)** · later: Oracle targets'),
];
...
const titleBar = new Table({ borders: tableNoBorders, layout: TableLayoutType.FIXED, width: { size: CONTENT_W, type: WidthType.DXA }, columnWidths: [CONTENT_W],
  rows: [new TableRow({ children: [new TableCell({ borders: noBorders, width: { size: CONTENT_W, type: WidthType.DXA },
    ...
    children: [new Paragraph({ spacing: { before: 0, after: 0 }, children: [
      ...
      new TextRun({ text: 'SQL Server → Aurora PostgreSQL · Amazon Redshift · Iceberg on S3 with Kiro  ·  8 skills · 9 steering files · 2 agents (conversion, reporting)  ·  Windows · Linux · macOS  ·  September 2026', font: FONT, size: 15, color: 'CFE3E7' }),
    ] })] })] })] });
